...
SELECT f.title, COUNT(a.`Actor Name`) AS 'Number of Actors in Film' 
FROM film f
JOIN actor a 
...
-- 6d    
SELECT title, COUNT(title) AS "Copies Count" 
FROM film
JOIN inventory 
...
a.`name` AS 'Film Category', 
a.category_id AS 'Family Category ID'
FROM film f
...
-- 7f
SELECT staff.store_id AS 'Store', 
(SELECT SUM(payment.amount) 
FROM payment 
WHERE payment.staff_id = staff.staff_id) AS 'Total Sales per Store'
FROM staff
...
-- 8a
CREATE VIEW `Top 5 Genres by Gross Revenue` AS SELECT `name`, SUM(amount) 
FROM category 
JOIN film_category 
ON category.category_id = film_category.category_id
JOIN inventory 
